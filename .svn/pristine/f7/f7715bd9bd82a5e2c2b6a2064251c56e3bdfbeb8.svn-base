...
public class SspPlanController extends BaseController {
	...
	public String edit(Long parentPlanId, String orderId, String advertiserId, Long planId, Integer isPlanGroup,
			Model model) {
			...
			try {
				...
				if (restResponse.isSuccess()) {
					...
					if (!sspOrderList.isEmpty()) {
						...
						model.addAttribute("sspOrder", sspOrder);
				...
				if (restResponse.isSuccess()) {
					...
					if (CommonUtil.isNotNull(sspplan)) {
						...
						if (CommonUtil.isNotNull(sspplan.get("sspPlanTargetCondition"))) {
							...
							// 根据策略id取定向属性
							RestResponse response = planTargetConditionService
									.find(Long.parseLong(plan.get("id").toString()));
							ArrayList<Object> targetCondition = (ArrayList<Object>) response.getResult();
							LOG.info(
									"edit planTargetCondition  size  -----------------------" + targetCondition.size());
							...
							if (targetCondition != null && targetCondition.size() != 0) {
								...
								// 时间定向
								planTargetConditionVo.setTimeTarget(contion.get("timeTarget") == null ? null
										: contion.get("timeTarget").toString());
								// 性别
								planTargetConditionVo.setSex(contion.get("sex") == null ? null
										: Long.parseLong(contion.get("sex").toString()));
								// 网络类型
								planTargetConditionVo.setNetworkType(contion.get("networkType") == null ? null
										: contion.get("networkType").toString());
								// 投放节奏
								planTargetConditionVo.setPutRhythm(Long.parseLong(contion.get("putRhythm").toString()));
								// 设备id定向控制
								planTargetConditionVo
										.setDeviceTypeState(Long.parseLong(contion.get("deviceTypeState").toString()));
								// 设备ID定向
								planTargetConditionVo.setDeviceTypeIdfa(contion.get("deviceTypeIdfa") == null ? null
										: contion.get("deviceTypeIdfa").toString());
								// 媒体ID定向
								planTargetConditionVo.setMediaIds(
										contion.get("mediaIds") == null ? null : contion.get("mediaIds").toString());
								// 地域类型状态
								planTargetConditionVo
										.setAreaTargetState(Long.parseLong(contion.get("areaTargetState").toString()));
								// 操作系统状态
								planTargetConditionVo.setDictSysOperationState(
										Long.parseLong(contion.get("dictSysOperationState").toString()));
								// 媒体类型状态
								planTargetConditionVo
										.setMediaState(Long.parseLong(contion.get("mediaState").toString()));
								// 广告类类型-定向子表单独获取
								LinkedHashMap sspDictAdType = (LinkedHashMap) contion.get("sspDictAdType");
								if (sspDictAdType.get("code") != null) {
									planTargetConditionVo
											.setSspDictAdTypeId(Long.parseLong(sspDictAdType.get("code").toString()));
								}
								// 地域类型-定向子表单独获取
								...
								// 媒体ID有无控制
								int mediastate = 1;
								if (CommonUtil.isNull(planTargetConditionVo.getMediaIds())
										&& planTargetConditionVo.getMediaIds().length() == 0) {
									mediastate = 0;
								}
								// 设备ID有无控制
								int deviceType = 1;
								if (CommonUtil.isNull(planTargetConditionVo.getDeviceTypeIdfa())
										|| planTargetConditionVo.getDeviceTypeIdfa().length() == 0) {
									deviceType = 0;
								}
	...
	@ResponseBody
	public RestResponse list3(String state, String planId, String idOrName, Long orderId, Integer currentPageNo,
			String pushStartTime, String pushEndTime) {
		...
		try {
			...
			RestResponse result = sspPlanService.page(orderId, startTime, endTime, planState, id, name, currentpage,
					pageSize);
	...
	public String goPlanListByOrderId(Long orderId, String advertiserId, String parentPlanId, Model model,
			HttpSession session) {
		model.addAttribute("advertiserId", advertiserId);
		model.addAttribute("orderId", orderId);
		model.addAttribute("parentPlanId", parentPlanId);
	...
	public RestResponse add(SspPlan sspPlan, HttpServletRequest request,
			@RequestParam(value = "putStartTimeStr", required = false) String putStartTimeStr,
			@RequestParam(value = "putEndTimeStr", required = false) String putEndTimeStr, String[] cityInfoArrStr,
			String[] systemType, String[] mediaType, Long planGroupFlag) {
		...
		try {
			...
			if (CommonUtil.isNotNull(sspPlan.getSspPlanTargetCondition().getNetworkType())) {
				sspPlan.getSspPlanTargetCondition()
						.setNetworkType(sspPlan.getSspPlanTargetCondition().getNetworkType());
			} else {
	...
	public RestResponse update(SspPlan sspPlan, HttpServletRequest request,
			@RequestParam("putStartTimeStr") String putStartTimeStr,
			@RequestParam("putEndTimeStr") String putEndTimeStr, String[] cityInfoArrStr, String[] systemType,
			String[] mediaType, Long planGroupFlag) {
		...
		try {
			...
			// 1是策略组，2策略（有定向更新）
			if (planGroupFlag == 2) {
				LOG.info("update SspPlanTargetCondition:--------------------"
						+ sspPlan.getSspPlanTargetCondition().getId());
			...
			if (CommonUtil.isNotNull(sspPlan)) {
				...
				if (sspPlan.getPutTimeState() == 1) {
					sspPlan.setPutStartTime(
							DateUtil.parseWithYYYYMMDDHHMMSS(String.format("%s 00:00:00", putStartTimeStr)));
					sspPlan.setPutEndTime(
							DateUtil.parseWithYYYYMMDDHHMMSS(String.format("%s 23:59:59", putEndTimeStr)));
				}
			}
			sspPlan.setUpdateTime(new Date());
	...
	public RestResponse findByAdvertiserId(Long advertiserId) {
		...
		try {
			if (CommonUtil.isNull(advertiserId))
	...
	public RestResponse changeState(SspPlan planParam, HttpServletRequest request) throws Exception {
	...
	public RestResponse searchPlanChildren(SspPlan planParam, Integer pageNum, HttpServletRequest request,
			HttpSession session) throws Exception {

		LOG.info("searchPlanChildren planid:{} currentPage:{}", planParam.getId(), pageNum);
		try {
			if (planParam != null && planParam.getId() != null) {
				// 根据id查找子策略
				RestResponse result = sspPlanService.searchPlanChildren(planParam, pageNum);
	...
	public String getCreativeListByPlanId(Long planId, Long advertiserId, Model model, HttpSession session) {
	...
	public RestResponse searchPlanCreativeRelationPage(HttpServletRequest request,
		...
		try {
			...
			RestResponse response = sspPlanService.searchPlanCreativeRelationPage(planId, currentPageNo, pageSize,
					creativeName, advertiserId, state, creativeType, creativeLabel);
			return response;
